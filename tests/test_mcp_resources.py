...
def test_logs_request_resource_registered():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource_uris = [res.uri for res in resources]
    assert "logs://request/{request_id}" in resource_uris
...
def test_logs_session_resource_registered():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource_uris = [res.uri for res in resources]
    assert "logs://session/{session_id}" in resource_uris
...
def test_logs_source_resource_registered():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource_uris = [res.uri for res in resources]
    assert "logs://source/{source}" in resource_uris


def test_recent_logs_resource_returns_json():
    """Test that logs://recent resource returns valid JSON."""
    resources = list(mcp._resource_manager._resources.values())
    resource = next(res for res in resources if res.uri == "logs://recent")
...
def test_error_logs_resource_returns_json():
    """Test that logs://errors resource returns valid JSON."""
    resources = list(mcp._resource_manager._resources.values())
    resource = next(res for res in resources if res.uri == "logs://errors")
...
def test_request_resource_accepts_request_id():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource = next(
        res for res in resources if res.uri == "logs://request/{request_id}"
    )

    # Check function signature
    import inspect
    sig = inspect.signature(resource.fn)
...
def test_session_resource_accepts_session_id():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource = next(
        res for res in resources if res.uri == "logs://session/{session_id}"
    )
...
def test_source_resource_accepts_source():
    ...
    resources = list(mcp._resource_manager._resources.values())
    resource = next(
        res for res in resources if res.uri == "logs://source/{source}"
    )
...
def test_request_resource_returns_json_with_test_id():
    ...
    result = resource.fn(request_id="test-request-123")
